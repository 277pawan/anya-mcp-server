...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', time: new Date().toISOString(), service: 'anya-mcp-server' });
});
...
async function testMapsMCP() {
  ...
  const placesResult = await searchNearbyPlaces('Connaught Place, New Delhi', 2000, 'hospital');
...
async function testAIRouter() {
  ...
  const testCases = ['hey what is your name?', 'find hospitals near Connaught Place'];
...
async function boot() {
  try {
    ...
    server.listen(PORT, () => {
      ...
      console.log(`WebSocket         → ws://localhost:${PORT}/ws/chat/:sessionId`);
      ...
      console.log('  GET      /api/history/mcp-calls | /ai-calls | /leads | /notifications\n');
    });
  } catch (err) {
